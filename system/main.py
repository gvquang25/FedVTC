...
from flcore.servers.serveravg import FedAvg
from flcore.servers.serverlag import FedLAG
from flcore.servers.serverflame import FLAME
from flcore.servers.serverpFedMe import pFedMe
from flcore.servers.serverperavg import PerAvg
from flcore.servers.serverprox import FedProx
...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        if model_str == "mlr":  # convex
            ...
            # Sử dụng kiến trúc ResNet18 nội bộ đã cập nhật hỗ trợ trả về (y, z) và tự điều chỉnh input channel
            in_ch = 1 if ("mnist" in args.dataset.lower() or "emnist" in args.dataset.lower()) else 3
            args.model = resnet18(num_classes=args.num_classes, in_channels=in_ch).to(args.device)
        ...
        elif args.algorithm == "FedVTC":
            # Khởi tạo trực tiếp FedVTC, giữ nguyên kiến trúc mô hình không bị chia tách
            server = FedVTC(args, i)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-lam', "--lamda", type=float, default=0.1,
                        help="Regularization weight (prototype regularization for FedVTC)")
    parser.add_argument('-mu', "--mu", type=float, default=0,
                        help="Proximal rate for FedProx")
    parser.add_argument('-K', "--K", type=int, default=5,
                        help="Number of personalized training steps for pFedMe")
    ...
    # FedVTC Hyperparameters
    parser.add_argument('-er', "--extra_rounds", type=int, default=50,
                        help="Số vòng extra fine-tuning với ảnh chưng cất")
    parser.add_argument('-zd', "--z_dim", type=int, default=512,
