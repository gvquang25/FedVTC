...
class ResNet(nn.Module):
    def __init__(
        ...
        has_bn = True,
        bn_block_num = 4,
        layers_branched: Optional[List[str]] = None,
    ) -> None:
        super(ResNet, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        ...
        # Hỗ trợ in_channels động (1 cho MNIST/EMNIST, 3 cho ảnh RGB)
    ...
    def get_name_of_shared_parameters(self, layers_branched: Optional[List[str]] = None) -> None:
        branched = set(layers_branched) if layers_branched else set()
        for name, param in self.named_parameters():
            top_level = name.split('.')[0]
            if top_level not in branched:
                self._shared_parameters[name] = param
    ...
    def _forward_impl(self, x: Tensor):
        ...
        # Nếu fc bị thay bằng nn.Identity (khi chạy FedAvg/FedLAG/BaseHeadSplit) -> trả về z đơn lẻ
        if isinstance(self.fc, nn.Identity):
            return z

        # Chạy thuật toán FedVTC -> trả về cặp (logits, latent_features)
        return y, z
